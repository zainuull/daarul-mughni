...
import useDataEvent from '../../store/store.events';
import { NotifyService } from '@/core/services/notify/notifyService';

export const UploadImage = () => {
  // const { deleteImage } = useViewModel();
  const [publicId, setPublicId] = useState('');
  const [data, setData] = useDataEvent();
  ...
  const handleUploadImage = (res: CldUploadWidgetResults) => {
    ...
    if ('secure_url' in info && 'public_id' in info) {
      ...
      setData({ ...data, public_id: public_id, imageUrl: url });
    ...
    notifyService.confirmationDelete().then(async (res) => {
      if (res) {
        try {
          ...
          if (res.ok) {
            setData({ ...data, public_id: '', imageUrl: '' });
          }
        } catch (error) {
          console.log(error);
        }
