...
import { IRoleDataModel } from '../../../domain/model/IModel';

const TableList = () => {
  const { getRoles, deleteRole } = useViewModel();
  ...
  const handleUpdate = async (data: IRoleDataModel) => {
    router.push(`/dashboard/role/update/${data?.id}`);
    setFormRole({
      ...formRole,
      id: data?.id,
      name: data?.name,
  ...
  return (
    <Table className="mt-5 min-h-[400px] relative pb-14 select-none">
      ...
      <TableBody>
        {result &&
          result?.map((data: IRoleDataModel) => (
